...
class NodeWorker(QRunnable, QObject):
    ...
    def __run(self, node):
        try:
            ...
            print(f"_____________________ERROR in thread, error: {error}, node: {node}")
            ...
            for i, (child_node, cur_data) in enumerate(it.product(
                ...
                if i > 0:
                    print("___________NEW_______________", self.uid)
                    self.__run_in_new_thread(child_node, cur_data)
                else:
                    print("___________OLD_______________", self.uid)
                    self.data = cur_data
                    self.__run(child_node)
                
    def __run_in_new_thread(self, node, data):
        new_worker = NodeWorker(node, deepcopy(data))
